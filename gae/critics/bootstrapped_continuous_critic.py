...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def __init__(self, hparams):
        ...
        self.critic_network = ptu.build_critic_mlp(
            self.ob_dim,
            1,
        ...
        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            if i%self.num_grad_steps_per_target_update==0:
                with torch.no_grad():
                    ...
                    target = reward_n + self.gamma * V_sp * -(terminal_n-1)

            self.optimizer.zero_grad()
            V_s_pred = self.critic_network(ob_no).squeeze()
            loss = self.loss(V_s_pred, target)
            loss.backward()
            self.optimizer.step()
